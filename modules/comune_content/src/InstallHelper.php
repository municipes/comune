...
namespace Drupal\comune_content;
...
class InstallHelper implements ContainerInjectionInterface {
  ...
  public function __construct(
    ...
    $this->baseUrl = 'https://municipes.github.io/origo/';
  }
  ...
  private function getJsonData($path) {
    ...
    try {
      ...
      Error::logException('comune_content', $e->getMessage());
    }
  ...
  protected function getModulePath() {
    $this->module_path = $this->moduleHandler->getModule('comune_content')->getPath();
    return $this;
  }
  ...
  protected function importContentFromFile($entity_type, $bundle_machine_name) {
    $path = $entity_type . '/' . $bundle_machine_name . '.json';
  ...
  public function deleteImportedContent() {
    $uuids = $this->state->get('comune_content_uuids', []);
  ...
  protected function storeCreatedContentUuids(array $uuids) {
    $uuids = $this->state->get('comune_content_uuids', []) + $uuids;
    $this->state->set('comune_content_uuids', $uuids);
  }
