...
class LogRotator {
  ...
  public function shouldRun(int $now): bool {
    $last = $this->state->get('silfi_log.rotation');
  ...
  protected function rotateFile(): bool {
    ...
    $timestamp = $this->state->get('silfi_log.rotation');
    ...
    if (!$truncate) {
      ...
      $destination = $this->config->get('file.location') . '/' . $destination;
